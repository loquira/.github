<h1 align="center">Loquira</h1>

<p align="center">
  <strong>Speak once. Reach everyone.</strong><br>
  Realtime speech translation and transcription.<br>
  One speaker, N listeners — each hearing in their own language.
</p>

<p align="center">
  <em>Coming soon.</em>
</p>

---

> Translation is not a barrier — it is a bridge.

---

## How it works

1. **Speaker** opens a session → gets a short code + QR code
2. **Listeners** scan the QR or enter the code → pick one of 200+ languages
3. **Speak naturally** — every listener hears the speech in their language with sub-second latency, in 50+ languages

---

## Capabilities

### Real-time Translation
Speak naturally — your audience hears in their own language with sub-second latency.

### Live Transcription
Every word is transcribed. Full session transcripts available for download.

### Simple Setup
One short code or QR scan. No app install needed. Works on any device.

### 200+ Languages
Translate into 200+ languages, with 50+ languages have natural-sounding neural voices.

### Sub-second Latency
Industrial-grade speech and translation pipelines keep speech-to-translation under one second.

### Downloadable Transcripts
Full session export — every word, in every language. Yours to keep.

---

## Where Loquira serves

- **Diplomatic missions** — Multilingual briefings without an interpreter booth — every attendee follows in their working language.
- **International conferences** — Every delegate follows the keynote in their native language, captioned and translated in real time.
- **Multilingual classrooms** — Lectures translated live for international students; transcripts available for revision after class.
- **Live broadcasting** — Captioned, translated broadcasts at production cadence — extend reach without an additional studio.

---

## Pricing

Billing unit: **language-hour** = one output language active for one hour. Audience size does not affect cost.

No card required. Start free, scale when ready.

---

<p align="center">
  <strong>Loquira</strong><br>
  Realtime translation for the world's most important conversations.<br><br>
  <strong>Going live soon.</strong>
</p>
